...
semantic html gives meaning to the reader as well as search engines so it's important to give semantics html.

2. Name two big differences between ```display: block;``` and ```display: inline;```.  1) block starts in the next line. inline doesnt
             2) you cannot change width in inline elements by default.
